...
# =====================================================
# 9️⃣ PLANNING DE LIVRAISONS FINAL
# =====================================================
st.markdown("## 📋 PLANNING DE LIVRAISONS FINAL")

if 'df_voyages_valides' in st.session_state and not st.session_state.df_voyages_valides.empty:
    
    # Créer une copie pour le planning
    df_planning = st.session_state.df_voyages_valides.copy()
    
    # Fonction pour formatter les colonnes avec retours à la ligne POUR EXCEL
    def formater_colonnes_listes_excel(df):
        df_formate = df.copy()
        colonnes_a_formater = ['Client(s) inclus', 'Représentant(s) inclus', 'BL inclus']
    ...
    df_planning_formate = formater_colonnes_listes_excel(df_planning)
    
    # Afficher le planning avec le même formatage que Voyages_Attribués
    st.markdown("### 📊 Planning de Livraisons Final")
    
    for idx, row in df_planning_formate.iterrows():
        with st.expander(f"🚚 Voyage {row['Véhicule N°']} | Zone : {row['Zone']} | Véhicule: {row.get('Véhicule attribué', 'N/A')} | Chauffeur: {row.get('Chauffeur attribué', 'N/A')}"):
            col1, col2, col3 = st.columns(3)
    ...
    # =====================================================
    # EXPORT COMPLET AVEC TOUTES LES FEUILLES
    # =====================================================
    ...
    with col_export2:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("🚀 Générer l'export complet avec toutes les feuilles", type="primary"):
            try:
                from backend import exporter_planning_excel
                
                # Préparer les données supplémentaires pour TOUTES les feuilles
                ...
                if 'df_city' in st.session_state and st.session_state.df_city is not None:
                    donnees_supplementaires['Besoin_Estafette_Ville'] = st.session_state.df_city
                if 'df_zone' in st.session_state and st.session_state.df_zone is not None:
                ...
                df_export_formate = formater_colonnes_listes_excel(df_planning)
                
                # Générer l'export COMPLET avec toutes les feuilles
                success, message = exporter_planning_excel(
                    df_export_formate,  # Utiliser le DataFrame formaté avec retours à ligne
                    f"{nom_fichier}.xlsx",
                    donnees_supplementaires,
                    st.session_state.df_livraisons_original if 'df_livraisons_original' in st.session_state else None
                )
                ...
                if success:
                    ...
                    st.subheader("👁️ Aperçu du Planning Principal")
                    ...
                    if "Poids total chargé" in df_apercu.columns:
                        df_apercu["Poids total chargé"] = df_apercu["Poids total chargé"].map(lambda x: f"{x:.1f} kg" if pd.notna(x) else "")
                    if "Volume total chargé" in df_apercu.columns:
                        df_apercu["Volume total chargé"] = df_apercu["Volume total chargé"].map(lambda x: f"{x:.3f} m³" if pd.notna(x) else "")
                    
                    st.dataframe(df_apercu, use_container_width=True)
                    
                    # Afficher les feuilles supplémentaires
                    st.info(f"📑 **Feuilles incluses dans l'export :**")
                    feuilles = ["Planning_Livraisons"]  # Feuille principale
                    if donnees_supplementaires:
                        feuilles.extend(list(donnees_supplementaires.keys()))
                    ...
                    try:
                        with open(f"{nom_fichier}.xlsx", "rb") as file:
                            btn = st.download_button(
                                label="💾 Télécharger le planning complet (XLSX)",
                                data=file,
                        ...
                        st.error("❌ Fichier non trouvé. L'export a peut-être échoué.")
                ...
                st.error("💡 Assurez-vous que la fonction 'exporter_planning_excel' existe dans le backend")

    # =====================================================
    # EXPORT SIMPLIFIÉ (UNE SEULE FEUILLE)
    # =====================================================
    st.markdown("---")
    st.markdown("### 📄 Export Rapide (Planning uniquement)")
    
    col1, col2 = st.columns(2)

    with col1:
        # Export Excel simple avec retours à ligne
        def to_excel_planning_simple(df):
            df_export = df.copy()
            ...
            if "Volume total chargé" in df_export.columns:
                df_export["Volume total chargé"] = df_export["Volume total chargé"].round(3)
            if "Taux d'occupation (%)" in df_export.columns:
                df_export["Taux d'occupation (%)"] = df_export["Taux d'occupation (%)"].round(3)
            
            output = BytesIO()
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                df_export.to_excel(writer, index=False, sheet_name='Planning_Livraisons')
                
                # Appliquer le formatage des retours à ligne et CENTRAGE dans Excel
                workbook = writer.book
                worksheet = writer.sheets['Planning_Livraisons']
        ...
        st.download_button(
            label="💾 Télécharger le Planning seul (XLSX)",
            data=to_excel_planning_simple(df_planning),
            file_name="Planning_Livraisons_Simple.xlsx",
            mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            help="Contient uniquement la feuille Planning_Livraisons avec retours à ligne"
        )
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=to_pdf_planning_centered(df_planning),
            file_name="Planning_Livraisons.pdf",
            mime='application/pdf',
            help="Version PDF du planning de livraisons"
        )

else:
    st.warning("⚠️ Vous devez d'abord valider les voyages dans la section 7 et attribuer les véhicules dans la section 8.")
