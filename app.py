...
if 'df_voyages_valides' in st.session_state and not st.session_state.df_voyages_valides.empty:
    ...
    # --- Export Excel avec retours à ligne et CENTRAGE (identique à Voyages_Attribués) ---
    from io import BytesIO
    import openpyxl

    def to_excel_planning(df):
        df_export = df.copy()
    ...
    def to_pdf_planning_centered(df, title="Planning de Livraisons"):
        pdf = FPDF(orientation='L')
        pdf.add_page()
    ...
    with col1:
        st.download_button(
            label="💾 Télécharger le Planning (XLSX)",
            data=to_excel_planning(df_planning),
            file_name="Planning_Livraisons.xlsx",
            mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        )
