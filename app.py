...
if 'df_voyages_valides' in st.session_state and not st.session_state.df_voyages_valides.empty:
    ...
    if st.button("✅ Appliquer les attributions"):
        ...
        def to_pdf_improved(df, title="Voyages Attribués"):
            ...
            # Configuration des colonnes avec en-têtes plus lisibles
            col_config = {
                ...
                'Client(s) inclus': {'width': 35, 'header': 'Clients'},
                'Représentant(s) inclus': {'width': 30, 'header': 'Représentants'},
                'BL inclus': {'width': 40, 'header': 'Bordereaux'},
            ...
            # En-têtes avec bordures complètes
            ...
            for voyage_idx, (_, row) in enumerate(df_pdf.iterrows()):
                ...
                for line_idx in range(max_lines_per_voyage):
                    for i, col in enumerate(colonnes_existantes):
                        ...
                        if line_idx == 0: border += 'T'  # Bordure haute (première ligne)
                        if line_idx == max_lines_per_voyage - 1: border += 'B'  # Bordure basse (dernière ligne)
                        if border == '': border = 'LR'  # Bordures verticales seulement
        ...
        # Version alternative avec fusion des cellules pour les données répétées
        def to_pdf_merged_cells(df, title="Voyages Attribués"):
            ...
            # Configuration des colonnes avec en-têtes courts
            col_config = {
                'Zone': {'width': 14, 'header': 'Zone'},
                'Véhicule N°': {'width': 16, 'header': 'Véhicule'},
                'Poids total chargé': {'width': 20, 'header': 'Poids'},
                'Volume total chargé': {'width': 20, 'header': 'Volume'},
                'Client(s) inclus': {'width': 32, 'header': 'Clients'},
                'Représentant(s) inclus': {'width': 28, 'header': 'Représ.'},
                'BL inclus': {'width': 38, 'header': 'B.L.'},
                'Taux d\'occupation (%)': {'width': 16, 'header': 'Taux %'},
                'Véhicule attribué': {'width': 20, 'header': 'Véh. Attrib.'},
            ...
            # Données avec fusion visuelle
            pdf.set_font("Arial", '', 8)
            
            for voyage_idx, (_, row) in enumerate(df_pdf.iterrows()):
                # Calculer le nombre de lignes nécessaires pour ce voyage
                ...
                for line_idx in range(max_lines):
                    for i, col in enumerate(colonnes_existantes):
                        if col in list_columns:
                            ...
                            # Colonnes normales - afficher seulement sur la première ligne
                            if line_idx == 0:
                                content = str(row[col]) if pd.notna(row[col]) and str(row[col]) != 'nan' else ""
                            else:
                        ...
                        border = ''
                        if line_idx == 0: border += 'T'  # Bordure haute
                        if line_idx == max_lines - 1: border += 'B'  # Bordure basse
                        if i == 0: border += 'L'  # Bordure gauche
                        if i == len(colonnes_existantes) - 1: border += 'R'  # Bordure droite
        ...
        # Utiliser la version avec bordures améliorées
        st.download_button(
            label="📄 Télécharger le tableau final (PDF Amélioré)",
            data=to_pdf_merged_cells(df_attribution),
